...
function build_nextup(leaderboard, display_type, meta, ddr_pilot_data, show_position=false) {
    ...
    for (var x = leaderboard.length; x < 4; ++x) {
        let pilot_name = "-";       
        let flagImg = '/csi_toolkit/ddr_overlays/static/imgs/flags/it.png';
        let pilotImg = '/csi_toolkit/ddr_overlays/static/imgs/no_avatar.png';
        let teamImg = '/csi_toolkit/ddr_overlays/static/imgs/no_team.png';
...
function getFlagURL(pilot_id, ddr_pilot_data) {
    let flagImg = '/csi_toolkit/ddr_overlays/static/imgs/flags/' + getPilotFlag(pilot_id, ddr_pilot_data) + '.png';
    if (!imageExists(flagImg)) {
        flagImg = '/csi_toolkit/ddr_overlays/static/imgs/flags/it.png';
    }
    return flagImg;
}
...
function getPilotImgURL(pilot) {
    ...
    if (!imageExists(pilotImg)) {
        pilotImg = '/csi_toolkit/ddr_overlays/static/imgs/no_avatar.png';
    }
    return pilotImg;
}
...
function render_pilots(rotorhazard) {
    ...
    for (var i = 0; i < rotorhazard.event.pilots.length; i++) {
        ...
        if (document.getElementById('pilot_id_flag_' + pilot.pilot_id)) {
            document.getElementById('pilot_id_flag_' + pilot.pilot_id).innerHTML = '<img class="country_flag" src="/csi_toolkit/ddr_overlays/static/imgs/flags/' + country_flag + '.jpg">';
        }
    }
}
